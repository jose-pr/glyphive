"""Opt-in glyphive OCR model for Nimbus Mono PS.

Installing this distribution registers a ``tesseract-glyphive-courier`` OCR
provider (via the ``glyphive.ocr_providers`` entry point) that runs Tesseract
against a fine-tuned LSTM model trained on Nimbus Mono PS renderings of the
...
from .provider import CourierGlyphiveProvider

__all__ = ["CourierGlyphiveProvider"]

#: The font this model targets and the provider name it registers.
FONT = "Nimbus Mono PS"
PROVIDER_NAME = "tesseract-glyphive-courier"
