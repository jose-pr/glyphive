...
Plus a physical-scan regression case (Layer 2b) that runs the committed
``tests/fixtures/physical_scan/`` degraded-scan fixture through
``tesseract-glyphive`` and skips cleanly without that engine, and a Layer 3
per-character-confidence case -- see each test's docstring.
"""
...
_PHYSICAL_SCAN_FIXTURE_DIR = Path(__file__).parent / "fixtures" / "physical_scan"
...
# --------------------------------------------------------------------------- #
# Layer 2b: committed physical-scan regression fixture
# --------------------------------------------------------------------------- #
def test_physical_scan_fixture_restores_byte_for_byte(tmp_path):
    """The committed degraded-scan fixture restores byte-for-byte.

    ``tests/fixtures/physical_scan/`` (generated by its ``generate.py``, see
    that file for the exact deterministic command) is a small synthetic tree
    rasterized at 300 DPI and degraded with a fixed Gaussian blur + rotation
    to stand in for a real phone-camera scan, without committing any real
    user content. This guards that a future change to descan/engine
    preference/decode does not silently break real-scan restore -- unlike
    ``test_real_ocr_gate_restores_byte_for_byte`` above, the input here is
    pre-degraded pixels checked into the repo, not a clean rasterization.

    Skips cleanly when ``tesseract-glyphive`` is not installed, consistent
    with this file's other real-engine gates.
    """
    engine = "tesseract-glyphive"
    if not _engine_available(engine):
        pytest.skip(f"OCR engine {engine!r} is not installed on this runner")

    pages_dir = _PHYSICAL_SCAN_FIXTURE_DIR / "pages"
    payload_path = _PHYSICAL_SCAN_FIXTURE_DIR / "payload.bin"
    if not pages_dir.is_dir() or not payload_path.is_file():
        pytest.skip("physical-scan fixture is missing (run generate.py)")
    ...
    restored_bytes = _archive.archive_tree(out_dir)
    expected_bytes = payload_path.read_bytes()
